...
// The suite acts on the real estate, so it cannot run without one. Checking here — before
// spawning the MCP server — means a missing stack reports as one readable line instead of an
// uncaught ECONNREFUSED from whichever fetch happened to go first, and it leaves no orphaned
// broker listening behind the failure.
const reachable = await fetch(`${STACK}/api/topology`).then(
  (r) => r.ok,
  () => false,
);
if (!reachable) {
  console.error(`\nThe target stack is not answering on ${STACK}.\n`);
  console.error("  Start it in another terminal, then re-run this:\n");
  console.error("    npm run stack\n");
  console.error("  If it is running elsewhere, point the suite at it:\n");
  console.error("    TARGET_STACK_URL=http://host:port node scripts/e2e.ts\n");
  process.exit(1);
}
...
const proposed = parse(
  await client.callTool({
    ...
    arguments: {
      ...
      deployId: "dep-9f12",
      rationale:
        "Failing checkout traces terminate inside the dep-9f12 code path.",
      evidence: [
        {
          source: "traces",
          claim: "All failing spans terminate in dep-9f12 code.",
          strength: "causal",
        },
        {
          source: "metrics",
          claim: "Error rate rose at the dep-9f12 rollout.",
...
proposalId
  ...
  : fail("no proposal returned", proposed);
...
noToken.ok === false
  ? ok("execution without a valid token is refused")
  : fail("unapproved execution succeeded", noToken);
...
const approval = await (
  await fetch(`${BROKER}/api/approve`, {
    ...
    body: JSON.stringify({
      ...
      note: "Approved for the e2e check.",
    }),
  })
).json();
approval.token
  ? ok("human approval issued a token")
  : fail("approval did not issue a token", approval);
...
const other = parse(
  await client.callTool({
    ...
    arguments: {
      ...
      evidence: [
        { source: "metrics", claim: "Memory climbing.", strength: "causal" },
      ],
    },
  }),
);
...
executed.ok
  ? ok("approved execution succeeds")
  : fail("approved execution failed", executed);

const after = await (
  await fetch(`${STACK}/api/metrics?service=checkout-api`)
).json();
after.metrics[0].errorRatePct < 1
  ? ok(
      `symptom cleared (checkout-api error rate now ${after.metrics[0].errorRatePct}%)`,
    )
...
replay.ok === false
  ? ok("an approval token is single use")
  : fail("token replay succeeded", replay);
